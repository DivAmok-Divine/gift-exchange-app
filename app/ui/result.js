...
export function showPairedResult(winner) {
    ...
    const savedType = state.currentUser?.giftType || localStorage.getItem('giftType_' + state.currentUser?.phone);
    ...
    resultContainer.innerHTML = `
        <div id="capture-card" class="paired-card" style="position:relative; margin-top:20px; margin-bottom:20px; text-align:left; background:rgba(0,0,0,0.2); padding:18px; border-radius:12px; border:1px solid rgba(255,255,255,0.1);">
            ...
            <p style="margin-bottom:10px; font-size:1.05rem;"><strong>Name:</strong> <span style="color:#fff; font-weight:800; margin-left:5px;">${winner.fullName}</span></p>
            <p style="margin-bottom:10px; font-size:1.05rem;"><strong>Phone:</strong> <span style="color:#fff; font-weight:800; margin-left:5px;">${winner.phone}</span></p>
            
            <p style="margin-bottom:20px; font-size:1.05rem;"><strong>Gift type:</strong> <span id="capture-gift-value" style="color:var(--secondary-color); font-weight:800; margin-left:5px; ${giftStyle}">${giftLabel}</span></p>
            ...
            <div class="no-capture">
                ...
                <div style="display:flex; flex-direction:column; gap:10px;">
                    ...
                    ${isTest ? '<button class="btn" style="padding:12px; margin-top:10px; background:#444; border:none; color:#fff; border-radius:8px; font-weight:700; cursor:pointer;" onclick="window.resetWheel()">Clear test result</button>' : ''}
                </div>
            </div>
        </div>
    `;
    ...
    window.__selectGiftType = (type) => {
        ...
        if (val) {
            ...
            if (state.currentUser) {
                ...
                localStorage.setItem('giftType_' + state.currentUser.phone, type);
            }

            // Sync to Google Sheet
            if (state.currentUser && !state.testMode) {
                saveUserPairing(state.currentUser.phone, winner.phone, type);
            }
        }
    };
